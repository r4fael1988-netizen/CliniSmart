...
async function finalSync() {
  console.log("Iniciando sincronização FINAL do n8n com segurança total...");
  ...
  const workflow = {
    ...
    nodes: [
      ...
      },
      {
        parameters: {
          options: {
            systemMessage: "Você é a SOFIA, a Gestora Inteligente da Clínica MASTER. Sua missão é gerenciar o atendimento de forma autônoma.\n\nIDENTIDADE:\n- Use as ferramentas para consultar dados reais no CRM.\n- ferramentas: obter_dados_clinicos, get_patient_history, check_availability, agendar_consulta, cancelar_consulta.\n- Tom de voz: Humano e acolhedor."
          }
        },
      ...
      // TOOLS (Using Authentication Headers directly for reliability)
      {
        parameters: {
          method: "POST", url: `${CRM_BASE_URL}/api/whatsapp/tools/get-clinic-data`,
          sendHeaders: true,
          headerParameters: { parameters: [ { name: "Authorization", value: `Bearer ${WEBHOOK_SECRET}` } ] },
          sendBody: true,
  ...
  try {
    ...
    if (updateResponse.ok) { console.log("n8n Sincronizado com Sucesso!"); }
