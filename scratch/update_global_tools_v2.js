...
async function updateAllTools() {
  try {
    ...
    const updateNode = (searchName, endpoint, bodyParams) => {
      const node = workflow.nodes.find(n => n.name === searchName);
      if (!node) {
          console.log(`Node '${searchName}' not found.`);
          return;
      }
    ...
    // Dados base vindos do Trigger
    const baseClinicId = "={{ $node[\"CRM Webhook Trigger\"].json.corpo.clinicId || $node[\"CRM Webhook Trigger\"].json.corpo.dados.clinicId }}";
    const basePhone = "={{ $node[\"CRM Webhook Trigger\"].json.corpo.patientPhone || $node[\"CRM Webhook Trigger\"].json.corpo.dados.patientPhone }}";
    const basePatientId = "={{ $node[\"CRM Webhook Trigger\"].json.corpo.patientId || $node[\"CRM Webhook Trigger\"].json.corpo.dados.patientId }}";

    // 1. Obter Dados Clínicos
    updateNode("get_clinic_data", "/api/whatsapp/tools/get-clinic-data", {
      clinicId: baseClinicId
    });

    // 2. Histórico
    updateNode("get_patient_history", "/api/whatsapp/tools/get-patient-history", {
      clinicId: baseClinicId,
      patientPhone: basePhone
    });
    ...
    // 6. Enviar resposta ao CRM (Node final HTTP)
