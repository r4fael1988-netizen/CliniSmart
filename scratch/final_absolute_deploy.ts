...
async function finalDeploy() {
  console.log("Iniciando Deploy FINAL com TODAS as ferramentas...");

  const OPENAI_CRED_ID = "WREj7d8Xvscq2n9d";
  const CRM_AUTH_CRED_ID = "D2r9X6jMv5s8lYwP";
  ...
  const workflow = {
    ...
    nodes: [
      ...
      },
      {
        parameters: {
          options: {
            systemMessage: "Você é a SOFIA, a Gestora Inteligente da Clínica MASTER. Sua missão é gerenciar o atendimento, agendamentos e informações via WhatsApp de forma totalmente autônoma.\n\nIDENTIDADE:\n- Use ferramentas (tools) para consultar dados antes de responder.\n- ferramentas disponíveis: obter_dados_clinicos, get_patient_history, check_availability, agendar_consulta, cancelar_consulta.\n- Tom de voz: Humano, acolhedor e eficiente.\n\nDIRETRIZES:\n1. Sempre use 'get_patient_history' no início para personalizar o atendimento.\n2. Para agendar, peça os dados, use 'check_availability' e depois 'agendar_consulta'.\n3. Confirme sempre os dados com o cliente antes de finalizar agendamentos."
          }
        },
      ...
      {
        parameters: { sessionId: "={{ $json.patientPhone }}", contextWindowLength: 10 },
      ...
      // TOOLS
      {
        parameters: {
          ...
          authentication: "genericCredentialType", genericAuthType: "headerAuth", sendBody: true,
          bodyParameters: { parameters: [ { name: "clinicId", value: "={{ $node[\"CRM Webhook Trigger\"].json.clinicId }}" } ] }
        },
        id: "tool-data", name: "obter_dados_clínicos", type: "@n8n/n8n-nodes-langchain.toolHttpRequest", typeVersion: 1.1, position: [950, 150],
        credentials: { httpHeaderAuth: { id: CRM_AUTH_CRED_ID } }
      },
      {
        parameters: {
        ...
        id: "tool-hist", name: "get_patient_history", type: "@n8n/n8n-nodes-langchain.toolHttpRequest", typeVersion: 1.1, position: [950, 250],
        credentials: { httpHeaderAuth: { id: CRM_AUTH_CRED_ID } }
      },
      {
        parameters: {
          ...
          bodyParameters: { parameters: [ 
            { name: "clinicId", value: "={{ $node[\"CRM Webhook Trigger\"].json.clinicId }}" },
            { name: "date", value: "By Model (and is required)" },
            { name: "doctorId", value: "By Model" }
          ] }
        },
        id: "tool-avail", name: "check_availability", type: "@n8n/n8n-nodes-langchain.toolHttpRequest", typeVersion: 1.1, position: [950, 350],
        credentials: { httpHeaderAuth: { id: CRM_AUTH_CRED_ID } }
      },
      {
        parameters: {
          ...
          bodyParameters: { parameters: [ 
            { name: "clinicId", value: "={{ $node[\"CRM Webhook Trigger\"].json.clinicId }}" },
            { name: "patientPhone", value: "={{ $node[\"CRM Webhook Trigger\"].json.patientPhone }}" },
            { name: "patientName", value: "={{ $node[\"CRM Webhook Trigger\"].json.patientName }}" },
            { name: "date", value: "By Model (and is required)" },
            { name: "doctorId", value: "By Model (and is required)" },
            { name: "serviceId", value: "By Model" }
          ] }
        },
        id: "tool-sched", name: "agendar_consulta", type: "@n8n/n8n-nodes-langchain.toolHttpRequest", typeVersion: 1.1, position: [950, 450],
        credentials: { httpHeaderAuth: { id: CRM_AUTH_CRED_ID } }
      },
      {
        parameters: {
          ...
          bodyParameters: { parameters: [ 
            { name: "clinicId", value: "={{ $node[\"CRM Webhook Trigger\"].json.clinicId }}" },
            { name: "appointmentId", value: "By Model (and is required)" }
          ] }
        },
        id: "tool-cancel", name: "cancelar_consulta", type: "@n8n/n8n-nodes-langchain.toolHttpRequest", typeVersion: 1.1, position: [950, 550],
        credentials: { httpHeaderAuth: { id: CRM_AUTH_CRED_ID } }
      },
      {
        parameters: {
  ...
  try {
    ...
    if (updateResponse.ok) { console.log("Workflow FINAL ATUALIZADO com sucesso!"); }
    else { console.error("Erro no final deploy:", await updateResponse.json()); }
